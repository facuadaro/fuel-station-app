import { Box, Button, Grid, GridItem, Image, Heading, Stack } from '@chakra-ui/react'
...
function Product() {
    ...
    const clickGasoil = (typesGasoil, priceGasoil) => {
        localStorage.setItem("typeGasoil", typesGasoil)
    ...
    <>
        ...
        <Grid templateColumns="repeat(2, 1fr)" gap={9}>
            {typesGasoil.map((gasoil, index) => (
                <GridItem key={index} w="100%" borderRadius="10" padding="4" cursor="pointer" textAlign="center" boxShadow="0 4px 10px -3px rgb(117,117,117);" _hover={{
                ...
                onClick={() => clickGasoil(gasoil.type, gasoil.price)}>
                    <Box>
                        ...
                        <Box display="flex" justifyContent="space-evenly" alignItems="center" marginTop={4} marginBottom={3}>
        ...
        <Box marginTop="40px">
            <Stack direction="row" spacing={4} cursor="pointer" onClick={() => navigate("/surtidor")}>
